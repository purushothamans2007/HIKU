...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="fixed top-0 w-full lg:pl-64 z-[60] h-16 bg-[#121317]/80 backdrop-blur-xl border-b border-white/10 shadow-sm flex items-center justify-between px-4 sm:px-6">
      {/* Left: Branding for mobile & Search Bar */}
      <div className="flex items-center gap-4 flex-1">
        <span className="font-bold text-xl text-primary tracking-tight lg:hidden">
          hiku
        </span>
        ...
        <div className="relative w-full max-w-md hidden sm:block">
          ...
          <input
            ...
            className="w-full bg-[#292a2e]/70 border border-white/10 rounded-full py-2 pl-10 pr-9 text-xs sm:text-sm text-white placeholder-[#c1c6d7]/50 focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/50 transition-all"
          />
          {searchQuery && (
            <button 
          ...
          {searchResults && (searchResults.poles.length > 0 || searchResults.faults.length > 0) && (
            <div className="absolute top-12 left-0 w-full glass-panel rounded-xl p-3 shadow-2xl border border-white/15 max-h-96 overflow-y-auto z-50 space-y-3">
              {searchResults.poles.length > 0 && (
                <div>
                  <div className="text-[11px] font-bold text-primary uppercase tracking-wider mb-1.5 px-1">
                  ...
                  <div className="space-y-1">
                    {searchResults.poles.map(p => (
                      <button
                        ...
                        className="w-full flex items-center justify-between p-2 rounded-lg bg-white/5 hover:bg-white/10 text-left transition-colors"
                      >
                        <div>
                          <span className="font-mono font-bold text-xs text-primary mr-2">{p.id}</span>
                          <span className="text-xs text-white">{p.location}</span>
                        </div>
                        <span className={`text-[10px] uppercase font-bold px-1.5 py-0.5 rounded ${
                          p.status === 'active' ? 'bg-[#39dcd2]/20 text-[#39dcd2]' : 'bg-[#ffb4ab]/20 text-[#ffb4ab]'
                        }`}>
                          {p.status}
                        </span>
                      </button>
                  ...
                  <div className="space-y-1">
                    {searchResults.faults.map(f => (
                      ...
                      >
                        ...
                        <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-white/10 text-white">
                          {f.severity}
                        </span>
                      </button>
                    ))}
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="relative">
          <button
            ...
            className="p-2 text-[#c1c6d7] hover:text-white hover:bg-white/10 rounded-full transition-all relative"
            title="Notifications"
          >
            <Bell className="w-5 h-5" />
            {unreadCount > 0 && (
              <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-[#ffb4ab] rounded-full ring-2 ring-[#121317] animate-pulse" />
            )}
          </button>

          {/* Notifications Dropdown Panel */}
          {showNotifications && (
            <div className="absolute right-0 top-12 w-80 sm:w-96 glass-panel rounded-2xl p-4 shadow-2xl border border-white/15 z-50 animate-in fade-in zoom-in-95">
              <div className="flex items-center justify-between pb-3 border-b border-white/10">
                <div className="flex items-center gap-2">
                  <h3 className="font-semibold text-sm text-white">Live Grid Alerts</h3>
                  {unreadCount > 0 && (
                    <span className="bg-[#ffb4ab]/20 text-[#ffb4ab] text-[10px] font-bold px-1.5 py-0.5 rounded-full">
                      {unreadCount} new
                    </span>
                  )}
                </div>
                <button
                  ...
                  className="text-[#c1c6d7] hover:text-white"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="py-2 divide-y divide-white/5 max-h-80 overflow-y-auto">
                {notifications.map((alert) => (
                  <div
                    ...
                    className={`py-2.5 px-2 rounded-lg cursor-pointer transition-colors flex items-start gap-3 ${
                      !alert.read ? 'bg-white/5' : 'hover:bg-white/5'
                    }`}
                  >
        ...
        <button
          ...
          className="p-2 text-[#c1c6d7] hover:text-white hover:bg-white/10 rounded-full transition-all"
        ...
        <div className="relative ml-1">
          <button
            ...
            className="w-9 h-9 rounded-full overflow-hidden border border-white/20 hover:border-primary transition-all relative ring-2 ring-transparent hover:ring-primary/30"
          >
            <img
          ...
          {showProfileMenu && (
            <div className="absolute right-0 top-12 w-64 glass-panel rounded-2xl p-4 shadow-2xl border border-white/15 z-50">
              <div className="flex items-center gap-3 pb-3 border-b border-white/10">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-primary">Chief Grid Controller</p>
                </div>
              </div>
              <div className="py-2 text-xs text-[#c1c6d7] space-y-1">
                ...
                <div className="flex justify-between py-1">
                  ...
                  <span className="font-mono text-[#39dcd2]">LEVEL-4 ROOT</span>
                </div>
              </div>
            </div>
          )}
